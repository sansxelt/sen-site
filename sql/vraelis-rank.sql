...
-- When a user signs in with an email domain that matches a VERIFIED org domain, they may
-- request to join that org (or auto-join if the org explicitly enables it). This NEVER grants
-- workspace/project/billing/API access — only org-level membership at a safe role. Conservative
-- default: 'request' (admin must approve). Additive + idempotent; existing orgs default safely.
alter table v_organizations add column if not exists domain_join_mode    text not null default 'request'; -- disabled|request|auto_member
alter table v_organizations add column if not exists domain_default_role text not null default 'member';  -- member|viewer (never admin/billing_admin/owner)

create table if not exists v_organization_join_requests (
  id                  uuid primary key default gen_random_uuid(),
  organization_id     uuid not null references v_organizations(id) on delete cascade,
  user_id             text,                          -- set once the requester signs in (matched by email)
  email               text not null,                 -- lowercased requester email (admin display only; never in event metadata)
  status              text not null default 'pending', -- pending|approved|rejected|canceled
  requested_role      text not null default 'member', -- member|viewer (clamped server-side)
  decided_by_user_id  text,                          -- lowercased email of the admin who decided
  decided_at          timestamptz,
  created_at          timestamptz not null default now(),
  updated_at          timestamptz not null default now()
);
create index if not exists v_org_join_req_org_idx on v_organization_join_requests (organization_id, status);
...
-- ══════════════════════════════════════════════════════════════════════════════
-- AI Output Check (product pivot). A "check" is an instant AI evaluation of 1..N
-- versions of an AI-generated output. It is NOT a human test: no votes, no options
-- table, no collect/complete lifecycle. The whole structured result (per-criterion
-- scores, computed recommendation, line-level flags) lives in `result` jsonb, the
-- same way v_reports.results does. Kept in its OWN table so the AI product never
-- pollutes the human-vote decision package. Additive + idempotent (safe to re-run).
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════════
-- Calibration (product pivot, stage 3). Links an AI check to the human validation
-- test spawned from the SAME output, so we can measure how often the check's pick
-- matches what real people pick. The AI prediction is captured at link time (by
-- LETTER, because the AI predicts before human option ids resolve); the human
-- outcome is back-filled lazily when the validation test completes (NOT inside the
-- money-critical completeTest). This is the moat seed: it runs quietly and the
-- agreement rate is surfaced only once it's real. Additive + idempotent.
-- ══════════════════════════════════════════════════════════════════════════════
...
-- One validation per check (claimed up front so a concurrent request can't double-launch),
-- one calibration per test.
