...
export function nextAction(s: CommandState): NextAction {
  ...
  // 6. A repair was verified but the full critical verification is still owed -> earn READY.
  if (s.repairPendingFullVerify && s.criticalEligibleCount > 0) {
    return { label: "Run full critical verification", href: `${app}/contract`, why: "The repair is verified; run every critical flow to earn READY.", tone: "primary" };
  }
  // 7. A newer, different deployment exists that the current decision never tested.
  if (s.newerUnverifiedDeploy) {
    return { label: "Verify this deployment", href: `${app}/contract`, why: "A newer deployment exists; your last decision doesn't cover it.", tone: "primary" };
  }
  ...
  if (s.decision === "ready") {
    return { label: "Run a new pass", href: `${app}/contract`, why: "This deployment is verified. Run again after your next change.", tone: "quiet" };
  }
  // 10. Approved contract with runnable flows and no prior decision: the first launch.
  return { label: "Preview & run a Production Pass", href: `${app}/contract`, why: "Preview the cost and readiness, then launch your first pass.", tone: "primary" };
}
...
export function stateRibbon(s: CommandState): RibbonFact[] {
  ...
  if (s.criticalTotal > 0) out.push({ key: "cov", text: `${s.criticalPassed}/${s.criticalTotal} critical`, tone: s.criticalPassed === s.criticalTotal ? "good" : "warn" });
