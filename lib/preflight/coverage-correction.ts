...
export const defaultFlowCorrector: FlowCorrector = async ({ claim, requirements, flows, obligations, missing, pages, rolesAvailable }) => {
  ...
  const prompt =
    ...
    `- ${signInRule}\n` +
    `- Assert the outcome with assert_text, and target the element that DISPLAYS the value — the text containing BOTH the label and the value (e.g. "Current plan"), NOT a bare section heading like "Plan". assert_text checks that the expected value appears INSIDE the target element's own text, so targeting only a heading, or relying on the value appearing elsewhere on the page (like a "Get ${val}" upsell), will NOT prove it. expect must be EXACTLY "${val}". A generic assert_visible of the word does NOT count.\n` +
