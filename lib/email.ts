    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr><td style="padding-bottom:24px;">
          <span style="font-size:18px;font-weight:600;color:#f5f5f5;letter-spacing:-0.01em;">sansxel</span>
        </td></tr>
        <tr><td style="background:#0a0a0a;border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:32px;">
          ${content}
        </td></tr>
...
function passwordResetHtml(resetUrl: string) {
  return baseHtml(`
    <p style="margin:0 0 8px;font-size:13px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:#a3a3a3;">Password Reset</p>
    <h1 style="margin:0 0 16px;font-size:24px;font-weight:600;color:#f5f5f5;line-height:1.3;">Reset your password.</h1>
    <p style="margin:0 0 24px;font-size:14px;line-height:1.7;color:#d4d4d4;">We received a request to reset the password on your sansxel account. Click the button below to choose a new one. This link expires in one hour.</p>
    <a href="${resetUrl}" style="display:inline-block;background:#fff;color:#000;font-size:14px;font-weight:500;padding:12px 24px;border-radius:14px;text-decoration:none;">Reset password</a>
    ...
    <p style="margin:0;font-size:12px;line-height:1.6;color:#525252;word-break:break-all;">Or copy this link: ${resetUrl}</p>
  `);
}
...
function supportHtml(opts: {
  ...
  return baseHtml(`
    <p style="margin:0 0 8px;font-size:13px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:#a3a3a3;">Support Request</p>
    <h1 style="margin:0 0 24px;font-size:20px;font-weight:600;color:#f5f5f5;line-height:1.3;">${opts.subject}</h1>
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
      <tr><td style="padding:4px 0;font-size:13px;color:#a3a3a3;width:80px;">From</td><td style="padding:4px 0;font-size:13px;color:#f5f5f5;">${opts.name || "—"}</td></tr>
      <tr><td style="padding:4px 0;font-size:13px;color:#a3a3a3;">Email</td><td style="padding:4px 0;font-size:13px;color:#f5f5f5;">${opts.email}</td></tr>
    </table>
    <div style="background:#050505;border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:16px;">
      <p style="margin:0;font-size:14px;line-height:1.7;color:#d4d4d4;white-space:pre-wrap;">${opts.message}</p>
    </div>
  `);
}
