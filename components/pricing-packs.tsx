...
function CardPack({
  ...
  // Render order: back → mid → front (so front paints on top)
  const slots = [
  ...
  function styleFor(depth: number): React.CSSProperties {
    ...
    const transform = hovered
      ? isBack ? "rotate(-14deg) translateX(-56px) translateY(20px)"
      : isMid  ? "rotate(-5deg)  translateX(-28px) translateY(8px)"
               : "rotate(0deg)   translateY(-8px)"
      : isBack ? "rotate(-5deg)  translateX(-18px) translateY(10px)"
      : isMid  ? "rotate(-2.5deg) translateX(-9px) translateY(5px)"
               : "rotate(0deg)";
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3">
        ...
        {locked && (
          <span className="rounded-full border border-white/[0.08] bg-white/[0.04] px-2.5 py-0.5 text-[10px] text-neutral-500">
            {hovered ? "Select a plan →" : "Hover to preview"}
          </span>
        )}
      </div>
      ...
      >
        {slots.map(({ plan, planIdx, depth }) => (
          <div
            key={plan.key}
            style={styleFor(depth)}
            onClick={() => depth > 0 && setActiveIdx(planIdx)}
          >
            <PlanCard plan={plan} />
          </div>
        ))}
...
export function PricingPacks() {
  return (
    <div className="mt-16 flex flex-col items-center gap-12 sm:mt-20 lg:flex-row lg:items-start lg:justify-center lg:gap-16 xl:gap-24">
      <CardPack plans={basicPlans}    label="Basic Plans" />
      <CardPack plans={advancedPlans} label="Advanced & Corporate" locked />
    </div>
  );
}
