#!/usr/bin/env bash
# PRODUCTION migration of the ordered set.  THIS WRITES TO PRODUCTION.
#
# RUN THIS YOURSELF. One password prompt; an assistant shell has no tty and the password must never enter
# a transcript.
#
#   usage:  bash ops/prod-migrate.sh <dump.sql> --forward     apply  P4 -> P3-C -> P3-D -> H3
#           bash ops/prod-migrate.sh <dump.sql> --rollback    revert H3 -> P3-D -> P3-C -> P4
#           bash ops/prod-migrate.sh <dump.sql> --verify      read-only posture check, writes nothing
#
# It refuses to run unless the mode is stated explicitly. There is no default.
#
# WHAT MAKES THIS SAFE TO RUN AT ALL: this exact ordered set, against this exact schema, was applied to
# staging and rolled back to the frozen 5187-fact fingerprint with zero differences, with 19 security and
# application smoke checks passing. --forward changes nothing about what was rehearsed.
set -uo pipefail
...
[ "$MODE" = "--forward" ]  && say "  order: P4 -> P3-C -> P3-D -> H3"
[ "$MODE" = "--rollback" ] && say "  order: H3 -> P3-D -> P3-C -> P4   (P4 restores privileges exactly, and runs LAST)"
say
...
if [ "$MODE" = "--verify" ]; then
  ...
  VLOG="${DUMP_DIR}/prod-verify-$(date -u +%Y%m%dT%H%M%SZ).log"
  pg -v ON_ERROR_STOP=1 -f /sql/prod-verify.sql > "$VLOG" 2>&1
  chmod 600 "$VLOG"
...
say
say "  -- Phase 4: ${MODE#--} --"
if [ "$MODE" = "--forward" ]; then IDX="0 1 2 3"; else IDX="3 2 1 0"; fi
for i in $IDX; do
  ...
  if [ "$st" -ne 0 ]; then
    ...
    say
    say "    ROLL BACK NOW:  bash ops/prod-migrate.sh ${DUMP_FILE} --rollback"
    exit "$st"
  fi
  say "    [ok] ${label}"
done
...
VLOG="${DUMP_DIR}/prod-${MODE#--}-verify-$(date -u +%Y%m%dT%H%M%SZ).log"
if [ "$MODE" = "--forward" ]; then
  ...
  say "  FORWARD COMPLETE - ${BEFORE} -> ${AFTER} facts, ${P} checks passed."
  say "  Deploy the application only after this point, and keep the rollback command to hand:"
  say "      bash ops/prod-migrate.sh ${DUMP_FILE} --rollback"
else
  D=0
  if [ -f "${DUMP_DIR}/prod-acl-expected.txt" ]; then
    D="$(diff "${DUMP_DIR}/prod-acl-expected.txt" "${DUMP_DIR}/prod-after-rollback.txt" | grep -cE '^[<>]')"
  fi
  unset PGPASSWORD
