#!/usr/bin/env bash
# Follow-up rehearsal: the two RPCs the application calls that production still lacks.
#
# RUN THIS YOURSELF. One password prompt; an assistant shell has no tty and the password must never enter
# a transcript.
#
#   usage:  bash ops/staging-rpc-rehearsal.sh /tmp/vraelis-schema-XXXXXXXX/prod-public-schema.sql
#
# FORWARD ORDER   P4 -> P3-C -> P3-D -> H3 -> R1 -> R2
# ROLLBACK ORDER  R2 -> R1 -> H3 -> P3-D -> P3-C -> P4     (exact reverse)
#
# The first four are ALREADY APPLIED TO PRODUCTION. They are replayed here only to bring staging to the
# state production is actually in (3584 facts) before the two new ones land on top - rehearsing R1/R2
# against staging's 5187-fact baseline would be rehearsing against a state that no longer exists anywhere.
#
# R1 v_hold_credits and R2 v_bind_oauth_identity are called by the application over PostgREST and exist in
# neither production nor the applied set. Both call sites are guarded and fail CLOSED, so the paths return
# 503 rather than breaking - this makes them work.
#
...
say
say "  RPC FOLLOW-UP REHEARSAL - STAGING ONLY"
say "  host: ${STAGING_HOST}:${STAGING_PORT}   user: ${STAGING_USER}   ref: ${STAGING_REF}"
say "  forward:  P4 -> P3-C -> P3-D -> H3 -> R1 -> R2"
say "  rollback: R2 -> R1 -> H3 -> P3-D -> P3-C -> P4"
say
...
BASELINE="${DUMP_DIR}/rpc-baseline.txt"
...
EXPECTED_FWD="${DUMP_DIR}/rpc-expected-forward.txt"
...
REF_RESTORED="${DUMP_DIR}/rpc-ref-restored.txt"
...
D="$(diff "$BASELINE" "${DUMP_DIR}/rpc-before.txt" | grep -cE '^[<>]')"
[ "$D" -eq 0 ] || { diff "$BASELINE" "${DUMP_DIR}/rpc-before.txt" | grep -E '^[<>]' | head -10 | sed 's/^/      /'
...
FWD_DRIFT=1
if [ -z "$FAILED_AT" ]; then
  FWD_DRIFT="$(diff "$EXPECTED_FWD" "${DUMP_DIR}/rpc-after-forward.txt" | grep -cE '^[<>]')"
  if [ "$FWD_DRIFT" -eq 0 ]; then
    say "    [ok] end state matches the offline-proven expectation exactly: 0 differing facts"
  else
    diff "$EXPECTED_FWD" "${DUMP_DIR}/rpc-after-forward.txt" | grep -E '^[<>]' | head -12 | sed 's/^/      /'
...
if [ -z "$FAILED_AT" ]; then
  ...
  VER_LOG="${DUMP_DIR}/rpc-verify.log"
  pg -v ON_ERROR_STOP=1 -f /sql/staging-rpc-verify.sql > "$VER_LOG" 2>&1
  chmod 600 "$VER_LOG"
...
RESTORE_DRIFT="$(diff "$BASELINE" "${DUMP_DIR}/rpc-after-rollback.txt" | grep -cE '^[<>]')"

# == Phase 7: verdict ========================================================
say
say "  ============ RPC FOLLOW-UP REHEARSAL RESULT ============"
...
if [ "$RESTORE_DRIFT" -ne 0 ]; then
  diff "$BASELINE" "${DUMP_DIR}/rpc-after-rollback.txt" | grep -E '^[<>]' | head -15 | sed 's/^/      /'
...
say "    RPC FOLLOW-UP REHEARSAL PASSED"
say "      All six applied in order, every check passed, the reverse rollback restored the frozen"
