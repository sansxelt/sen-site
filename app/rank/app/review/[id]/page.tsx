...
export default async function ReviewPlanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Page measure="prose">
      {/* The back link used to be spaced by the heading's own 12px top margin. <PageHeader> owns the
          heading's metrics now and starts flush against whatever precedes it, so the link carries its own
          gap rather than reaching into the header for one. */}
      <div style={{ marginBottom: 12 }}>
        <Link href="/review" style={{ fontSize: 13, color: "var(--fg-4)", textDecoration: "none" }}>← Review</Link>
      </div>

      {/* The heading is the claim under review, so it is data rather than a fixed page name. It still goes
          through <PageHeader>: the reason six h1 sizes existed is that pages with something particular to
          say each argued themselves an exception, and a customer's sentence is not a reason to render the
          title of this page at a different size from the title of every other one. */}
      <PageHeader title={plan.claim || "Untitled claim"} />
