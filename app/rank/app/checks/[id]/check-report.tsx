  ...
  return (
    <div style={{ ...box, padding: "clamp(16px, 2.4vw, 22px)", ...(isWinner ? { borderColor: "var(--acc-line)", background: "var(--acc-soft)" } : {}) }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <span style={{ marginLeft: "auto", fontFamily: "var(--font-code)", fontSize: 12.5, color: scoreColor(c.overall) }}>Overall {c.overall}</span>
      </div>
    ...
    <div>
      ...
      <div style={{ ...box, background: "var(--bg-2)", padding: "clamp(16px, 2.4vw, 22px)" }}>
        ...
        <p style={{ fontSize: 12.5, color: "var(--fg-4)", lineHeight: 1.7, margin: 0 }}>
          This is an <strong style={{ color: "var(--fg-2)" }}>AI assessment</strong>{result.model ? ` generated by ${result.model}` : ""}, not a human judgment and not a guarantee. Every flagged span is quoted verbatim from your output, and the recommended version is computed from the scores shown, not asserted by the model. It informs your decision; it does not replace shipping judgment or real user testing.
        </p>
      </div>
    </div>
  );
