...
export default async function SharedReportPage({ params }: { params: Promise<{ testId: string }> }) {
  ...
  return (
    <div className="wrap" style={{ maxWidth: 820, paddingTop: "clamp(24px, 3vw, 40px)", paddingBottom: 80 }}>
      ...
      {test.status === "active" ? (
        <>
          <p style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--fg-4)", marginBottom: 14 }}>Evaluation in progress — {test.votes_valid} / {test.votes_target} valid judgments collected so far.</p>
