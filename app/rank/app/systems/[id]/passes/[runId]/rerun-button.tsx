...
export function RerunButton({ appId, runId, scope, label, priceNote }: { appId: string; runId: string; scope: "failed" | "all" | "critical"; label: string; priceNote?: string | null }) {
  ...
  async function run() {
    ...
    try {
      ...
      if (res.status === 401) { router.push(`/signin?callbackUrl=${encodeURIComponent(`/systems/${appId}/passes/${runId}`)}`); return; }
      if ((res.ok || res.status === 409) && j?.runId) { router.push(`/systems/${appId}/passes/${j.runId}`); return; } // navigating away; keep busy
