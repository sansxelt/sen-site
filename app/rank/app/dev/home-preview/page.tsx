...
// without an authenticated session or any production data. It is gated two ways: notFound() unless NODE_ENV
// is development, AND (because "dev" is not an APP_ROOT) the app-host proxy never rewrites to it in
// production, so it 404s there regardless. Final authenticated capture stays in Increment 5.
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import type { PassRow, IssueRow } from "@/lib/preflight/overview-db";
import { HomeAttention, RunningWork, RecentVerifications, SystemsSummary, EmptyHome, type SystemSummaryItem } from "../../_components/home-records";

export const metadata: Metadata = { title: "Home preview (dev)", robots: { index: false, follow: false } };

// Fixed timestamps (deterministic offsets from render time) keep the preview stable to review.
const H = 3600_000;
...
const RECENT: PassRow[] = [
  ...
  pass({ id: "r2", applicationName: "Checkout App", decision: "blocked", deploymentUrl: "https://checkout.example.app", flowsTotal: 6, flowsPassed: 4, completedAt: ago(5 * H) }),
