...
export default function DemoReport() {
  return (
    <div className="wrap" style={{ maxWidth: 820, paddingTop: "clamp(24px, 3vw, 40px)", paddingBottom: 80 }}>
      ...
      <h1 className="display" style={{ fontSize: "clamp(1.8rem, 3.2vw, 2.5rem)", marginBottom: 8 }}>Which response is more helpful, accurate, and safe?</h1>
      <p style={{ fontSize: 14.5, color: "var(--fg-3)", marginBottom: 14, lineHeight: 1.55 }}>A sample pairwise preference judgment between two model responses to the same prompt. It shows the exact output Vraelis returns — the preferred response, preference margin, directional confidence, signal quality, and reasoning signals. <strong style={{ color: "var(--fg-2)" }}>All figures below are illustrative demo data, not real judgments.</strong></p>
      ...
      <div className="card" style={{ textAlign: "center", borderColor: "var(--acc-line)", background: "var(--acc-soft)", padding: "clamp(24px, 4vw, 36px)" }}>
        <div className="display" style={{ fontSize: "clamp(1.4rem, 2.6vw, 1.9rem)", marginBottom: 8 }}>Run this on your own outputs.</div>
        <p style={{ fontSize: 14.5, color: "var(--fg-2)", maxWidth: 460, margin: "0 auto 20px", lineHeight: 1.55 }}>Submit your candidates, collect quality-filtered human judgments, and get a Decision Package like this one — in the app or by API.</p>
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <a href="/app/new" className="btn btn--lg">Start free →</a>
