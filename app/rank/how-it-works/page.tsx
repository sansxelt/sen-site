...
export const metadata = ogMeta({
  title: "How it works",
  description:
    "How Vraelis works: connect your AI-built app, approve what it must do, and Vraelis runs it in a real browser as two real users, then returns a launch decision with the exact blockers to fix.",
  path: "/how-it-works",
});
...
const STEPS: { k: string; t: string; d: string; i: string }[] = [
  ...
  { k: "05", t: "Get the Production Pass", d: "One decision: READY, NEEDS REVIEW, or BLOCKED. Each blocker carries its requirement, the expected and observed behavior, exact reproduction steps, and a screenshot.", i: I.shield },
  { k: "06", t: "Repair with proof", d: "Every blocker ships with a fix prompt for your builder. Push the fix and Vraelis reruns the exact failed check on a preview, then confirms the regression is closed.", i: I.wrench },
];
      ...
      <section style={{ position: "relative" }}>
        ...
        <div className="wrap" style={{ position: "relative", zIndex: 1, paddingTop: "clamp(48px, 6vw, 88px)", paddingBottom: "clamp(20px, 3vw, 34px)", textAlign: "center" }}>
          ...
          <p className="lead-copy" style={{ margin: "0 auto", textAlign: "center", maxWidth: 700 }}>
            No test scripts to write and no dashboard to babysit. You connect your app, approve what it must do, and Vraelis drives it in a real browser as two real users. You get back one launch decision and the exact blockers behind it.
          </p>
        ...
        <div className="wrap">
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: "clamp(28px, 4vw, 56px)", alignItems: "center" }} className="cols-stack">
            <div>
              ...
              <p style={{ fontSize: 14, color: "var(--fg-3)", lineHeight: 1.6 }}>The launch decision follows one explainable rule, not a number you have to trust: any critical flow that fails blocks the launch, any other failure needs review, and only a clean run is ready.</p>
            </div>
