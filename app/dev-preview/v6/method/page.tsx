...
export const metadata: Metadata = v6meta({
  ...
  description: "The worldview behind Vraelis: the guarantee before the implementation, evidence before confidence, and autonomy that must be earned.",
  path: "/method",
...
const CHAPTERS: [string, string, string[]][] = [
  ...
  // THE POSITION IS NAMED AFTER THE OBJECT THE PRODUCT ACTUALLY HAS.
  //
  // This chapter was "Responsibility before implementation", and the anchor was #responsibility. That noun
  // exists nowhere in the schema or the code: the durable object a person writes is a guarantee, and the
  // console, the docs and the platform page now all say so. A method page that names its first position
  // after a thing the product does not contain teaches a reader a word they will never see again. Nothing
  // linked to #responsibility, so the anchor moved with the noun; the only cross-page anchor into this file
  // is #in-public, used by /company and the shell.
  ["guarantee", "The guarantee comes before the implementation", [
  ...
  ["repair", "Repairs must resolve the guarantee", [
  ...
  // "Every responsibility teaches the system how a company's software and agents actually fail" was written
  // in the present tense about something _content/scope.ts files under Horizon and calls not built. These are
  // positions, so the tense is the fix: what oversight SHOULD become, not a description of what it already
  // does. /docs/memory now carries the same boundary in the same words.
  ["memory", "Memory should compound", [
